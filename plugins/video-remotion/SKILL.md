...
| STORYBOARD | Scene breakdown | Markdown storyboard (or `chapters.ts` entry once schema is stable) | Anchor flags assigned; beat timings sum to target |
| BUILD | Storyboard | Rendered video (1080×1920 + 2x + cover.png + caption) | Render verified via ffprobe + visual safe-area check |

**After the first 2-3 chapters, the storyboard markdown can collapse into an inline `chapters.ts`-style schema entry** — once the 5-beat structure and anchor-flag vocabulary stabilize, the structural decisions migrate into code comments + beat consts. This is iteration compression at the storyboard layer.
...
1. **Extend** the timeline (`durationInFrames += N`) so beats breathe
...
### 6. Text safe-area — 10px inset, full-bleed background

Wrap all text/UI in a positioned container:
...
{/* backgrounds render OUTSIDE the wrapper as full-bleed */}
```

**Failure mode:** large display fonts plus text-shadow stacks push the bounding box past the 1080×1920 edge on IG Reels. The clip is invisible in the Remotion preview but obvious on the phone.

**Account for text-shadow extent.** A 4px offset shadow needs text origin ≥14 from the edge, not 10. When in doubt, render the cover, view it on the actual posting device, look at the corners.

### 7. Render config — concurrency=2, capture exit codes, never pipe through tee

Default render config on a modern Mac workstation:

```bash
npx remotion render src/index.tsx ChapterReel out/chapter-N-v01-1080.mp4 \
  --props='{"chapter":"N"}' \
  --concurrency=2 \
  && EXIT=$?

# Verify the render actually completed
ffprobe -v error out/chapter-N-v01-1080.mp4 || EXIT=1
```

**Hard rules:**
- `--concurrency=2` default. Higher triggers `ProtocolError (Page.bringToFront): Target closed.` mid-render on Chromium.
- Capture `EXIT=$?` immediately after the chain. Never `&&` past it.
- **Never pipe through `tee`** — the actual render command's exit code gets masked by `tee`'s success. A crashed render reports `EXIT=0`.
- If the render crashes mid-frame: kill orphan Chromium (`pgrep -f Chromium | xargs -r kill -9`) before retry.
- If concurrency=2 still crashes: drop to `--concurrency=1`.

### 8. Version control — `vNN` from the FIRST render, never overwrite
...
### 9. Cover image generation — render alongside, not after
...
**Pattern:** add a `chapter-N-vN-cover.png` render output in the same Remotion config. Use the chapter's hero anchor (anchor-2's visual primitive at the frame where it's most legible) as the cover frame, not the title slide.
...
### 10. Iteration compression — encode Chapter 1's lessons at the top of the skill
...
3. **Templated** — `chapters.ts` (or equivalent) schema embeds the 5-beat structure so chapter authors can't accidentally skip beats
...
### 11. Humanizer-grep gate — empty grep is necessary, not sufficient
...
### 12. Failure-mode catalog — the negative corpus IS half the lesson

Every render thrown away taught something. Catalog them. The catalog itself is part of the skill.

Known failure-mode categories from production:

| Category | Symptom | Root cause | Discipline that prevents |
|---|---|---|---|
| Safe-area clip | Text clipped on cover or final slide | Display font + shadow exceeds 1080×1920 | #6 |
| Tee-masked render crash | Exit code 0 but video truncated mid-chapter | Pipe through tee swallowed exit code | #7 |
| Audio post-process violation | Chipmunk voice on v03 attempt | Reached for atempo to "fix pace" | #2 |
| Manuscript content trim | Teaching device removed to fit time budget | Confused "feels rushed" with "needs trim" | #4 |
| Density-floor miss | Beat 3 reads as dead air | No animation layer for 5s static slide | #3 |
| Stale-version overwrite | Lost a v02-shape decision | Overwrote v02 with v03 in place | #8 |
| Slogan-anchor | Anchor has no visual to attach to | Chose anchor at storyboard stage, not scene-breakdown | #5 |

**Pattern:** every failure mode maps to a discipline above. When a new failure type appears, add a row here AND add the prevention to the matching discipline.
...
- **Photoshop cover after render** — drift; see #9
- **Overwriting render output** — loses the snapshot; see #8
- **High concurrency to "speed up renders"** — target-closed crash costs more time than the parallelism saved; see #7
- **Em-dashes in TTS narration** — pause longer than commas, sound stagey, are AI-slop signature; see #11

## Worked examples

The three worked examples below show the pipeline applied to three different chapter shapes. The teaching content is abstracted to protect the source series; the pipeline mechanics are real production records.
...
**Stage 4 (STORYBOARD):** full markdown storyboard (~600 lines) — the heaviest storyboard of the series because primitives didn't exist yet. Three chapter-specific animation primitives identified for build: autocomplete-anim, distribution-curve-anim, oracle-vs-assistant-flow.
...
**Stage 4 (STORYBOARD):** **inline in `chapters.ts`** — no separate markdown storyboard. By this point in the series, the 5-beat structure and anchor-flag schema were stable enough that the structural decisions migrated to code comments + beat consts. This is iteration compression at the storyboard layer.
...
- `references/iteration-history.md` — per-chapter version count, word count, duration, wpm for the 8-chapter source series
- `references/anchor-flag-schema.md` — the reusable animation primitive vocabulary that emerged from chapters 1-4
- `references/humanizer-banned-tokens.md` — the full grep pattern + rationale for each token
